...
_call-rust_ is a function to call Rust's function.
This is defined as a built-in function as follows.
...
It takes 3 integers and returns (Option Int).
In order to call Rust's function, setting a callback function is required as follows.
...
<1> callback function called from BLisp. The function type must is Fn:(&BigInt, &BigInt, &BigInt) -> Option<BigInt>
...
_call-rust_ is a IO function. Therefore, this cannot be called from Pure functions.
